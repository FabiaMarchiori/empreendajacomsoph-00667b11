      ...
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
        ...
        <div className="flex items-center gap-2 pt-1">
          <div className="flex -space-x-2">
            {[0, 1, 2].map((i) => (
              <div key={i} className="h-7 w-7 rounded-full border-2 border-background flex items-center justify-center" style={{ background: "#00FFFF" }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {tips.map((tip) => (
            <div key={tip.title} className="rounded-xl border border-border/60 p-5 bg-gradient-card hover:border-primary/20 transition-all">
              <div className="h-10 w-10 rounded-lg flex items-center justify-center mb-3" style={{ background: "#00FFFF" }}>
