    ...
    <div className="p-4 sm:p-6 lg:p-10 max-w-6xl mx-auto space-y-8 sm:space-y-10">
        ...
        <motion.div variants={item} className="rounded-2xl p-5 sm:p-8 lg:p-10 border border-[#007A7A]/30 relative overflow-hidden min-h-[220px] sm:min-h-[260px]" style={{ background: 'linear-gradient(135deg, #102A43 0%, #0A192F 100%)' }}>
          ...
          <div className="relative z-10 max-w-lg">
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={() => navigate("/estruture")} className="px-5 py-2.5 rounded-xl text-[#0A192F] text-sm font-bold transition-all duration-300 hover:brightness-110 hover:shadow-[0_0_24px_-4px_rgba(0,239,255,0.5)] active:scale-95 flex items-center gap-2 shadow-[0_0_20px_-4px_rgba(0,239,255,0.4)]" style={{ background: GRAD }}>
                Continuar jornada <ArrowRight className="h-4 w-4" />
              </button>
              <button onClick={() => navigate("/acessos")} className="px-5 py-2.5 rounded-xl border-2 border-white/40 text-white text-sm font-bold transition-all duration-300 hover:bg-white/10 hover:border-white/60 active:scale-95">
                Ver meus acessos
              </button>
              <button onClick={() => navigate("/soph")} className="px-5 py-2.5 rounded-xl bg-gradient-primary-btn text-primary-foreground text-sm font-bold transition-all duration-300 hover:brightness-110 active:scale-95 flex items-center gap-2 shadow-glow-sm">
      ...
      <motion.section variants={container} initial="hidden" animate="show">
        <motion.div variants={item} className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => navigate("/acessos")} className="text-xs text-gradient-primary inline-block hover:underline font-bold tracking-wide transition-opacity hover:opacity-80">Ver todos</button>
        </motion.div>
        ...
        <motion.div variants={item} className="rounded-2xl border border-[#007A7A]/20 p-6 lg:p-8" style={{ background: 'linear-gradient(135deg, #102A43 0%, #0A192F 100%)' }}>
          ...
          <div className="relative">
            ...
            <div
              ...
              style={{ width: `${(1 / (journeySteps.length - 1)) * 100}%` }}
            />
            <div className="relative flex justify-between">
              {journeySteps.map((step) => (
                <div key={step.label} className="flex flex-col items-center text-center min-w-0" style={{ width: `${100 / journeySteps.length}%` }}>
                  <div
                    className={`h-10 w-10 rounded-full flex items-center justify-center border-2 relative z-10 transition-all duration-300 ${
        ...
        <motion.div variants={item} className="rounded-2xl border border-[#00EFFF]/25 p-6 shadow-[0_0_25px_-8px_rgba(0,239,255,0.15)] relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #102A43 0%, #0A192F 100%)' }}>
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <button onClick={() => navigate("/gestao")} className="px-5 py-2 rounded-xl text-[#0A192F] text-sm font-bold transition-all duration-300 hover:brightness-110 hover:shadow-[0_0_24px_-4px_rgba(0,239,255,0.5)] active:scale-95 flex items-center gap-2 shadow-[0_0_20px_-4px_rgba(0,239,255,0.4)]" style={{ background: GRAD }}>
      ...
      <motion.section variants={container} initial="hidden" animate="show">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            { title: "Semi-jóias de Limeira", desc: "Acesse fabricantes direto do polo de Limeira", icon: <Gem className="h-5 w-5" /> },
            { title: "Calçados dos Polos", desc: "Conecte-se com fábricas de calçados do Brasil", icon: <ShoppingBag className="h-5 w-5" /> },
      ...
      <motion.section variants={container} initial="hidden" animate="show">
        <motion.div variants={item}>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {[
              { label: "Retomar onde parei", icon: <RotateCcw className="h-4 w-4" />, action: () => {
                const last = localStorage.getItem("empreendaja_last_page");
                navigate(last && last !== "/" ? last : "/fornecedores");
              }},
              { label: "Meus favoritos", icon: <Heart className="h-4 w-4" />, action: () => navigate("/fornecedores/importadoras-25/favoritos") },
              { label: "Simulações recentes", icon: <History className="h-4 w-4" />, action: () => navigate("/gestao/precificacao") },
              { label: "Falar com suporte", icon: <HelpCircle className="h-4 w-4" />, action: () => navigate("/soph") },
            ].map((a) => (
              <button
                key={a.label}
                onClick={a.action}
                className="flex items-center gap-2.5 p-3.5 rounded-xl border border-white/10 text-sm text-white font-semibold transition-all duration-300 hover:border-[#00EFFF]/30 hover:shadow-[0_0_12px_-4px_rgba(0,239,255,0.2)] hover:bg-white/[0.03] active:scale-[0.97]"
