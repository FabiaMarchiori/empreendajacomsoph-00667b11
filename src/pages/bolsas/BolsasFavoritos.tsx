...
export default function BolsasFavoritos() {
  const navigate = useNavigate();
  const { favorites, toggle, isFavorite, isLoading: loadingFavs } = useSupabaseFavorites();
  const { data: allSuppliers, isLoading: loadingSuppliers } = useBolsasSuppliers();

  const isLoading = loadingFavs || loadingSuppliers;
  const favSuppliers = (allSuppliers || []).filter((s) => s.allIds.some((id) => favorites.includes(id)));
